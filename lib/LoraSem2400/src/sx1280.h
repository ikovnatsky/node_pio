...
}HFRadioRangingIdCheckLengths_t;
...
{
    HFRadioTickSizes_t PeriodBase;                            //!< The base time of ticktime
    /*!
...
*/
#define RX_TX_CONTINUOUS ( HFTickTime_t ){ RADIO_TICK_SIZE_0015_US, 0xFFFF }
#define RX_TX_SINGLE     ( HFTickTime_t ){ RADIO_TICK_SIZE_0015_US, 0 }
...
typedef struct
{
	HFRadioPacketTypes_t                    PacketType;       //!< Packet to which the modulation parameters are referring to.
    struct
    {
        /*!
        ...
        struct
        {
        	HFRadioGfskBleBitrates_t    BitrateBandwidth;     //!< The bandwidth and bit-rate values for BLE and GFSK modulations
        	HFRadioGfskBleModIndexes_t  ModulationIndex;      //!< The coding rate for BLE and GFSK modulations
        	HFRadioModShapings_t        ModulationShaping;    //!< The modulation shaping for BLE and GFSK modulations
        }Gfsk;
        /*!
        ...
        struct
        {
        	HFRadioLoRaSpreadingFactors_t  SpreadingFactor;   //!< Spreading Factor for the LORA modulation
        	HFRadioLoRaBandwidths_t        Bandwidth;         //!< Bandwidth for the LORA modulation
        	HFRadioLoRaCodingRates_t       CodingRate;        //!< Coding rate for the LORA modulation
        }LoRa;
        /*!
        ...
        struct
        {
        	HFRadioFlrcBitrates_t          BitrateBandwidth;  //!< The bandwidth and bit-rate values for FLRC modulation
        	HFRadioFlrcCodingRates_t       CodingRate;        //!< The coding rate for FLRC modulation
        	HFRadioModShapings_t           ModulationShaping; //!< The modulation shaping for FLRC modulation
        }Flrc;
        /*!
...
typedef struct
{
	HFRadioPacketTypes_t                    PacketType;       //!< Packet to which the packet parameters are referring to.
    struct
    {
        /*!
        ...
        struct
        {
        	HFRadioPreambleLengths_t       PreambleLength;    //!< The preamble length for GFSK packet type
        	HFRadioSyncWordLengths_t       SyncWordLength;    //!< The synchronization word length for GFSK packet type
        	HFRadioSyncWordRxMatchs_t      SyncWordMatch;     //!< The synchronization correlator to use to check synchronization word
        	HFRadioPacketLengthModes_t     HeaderType;        //!< If the header is explicit, it will be transmitted in the GFSK packet. If the header is implicit, it will not be transmitted
            uint8_t                      PayloadLength;     //!< Size of the payload in the GFSK packet
            HFRadioCrcTypes_t              CrcLength;         //!< Size of the CRC block in the GFSK packet
            HFRadioWhiteningModes_t        Whitening;         //!< Usage of whitening on payload and CRC blocks plus header block if header type is variable
        }Gfsk;
        /*!
        ...
        struct
        {
            ...
            HFRadioLoRaCrcModes_t           Crc;              //!< Size of CRC block in LORA packet
            HFRadioLoRaIQModes_t            InvertIQ;         //!< Allows to swap IQ for LORA packet
        }LoRa;
        /*!
        ...
        struct
        {
        	HFRadioPreambleLengths_t       PreambleLength;    //!< The preamble length for FLRC packet type
        	HFRadioFlrcSyncWordLengths_t   SyncWordLength;    //!< The synchronization word length for FLRC packet type
        	HFRadioSyncWordRxMatchs_t      SyncWordMatch;     //!< The synchronization correlator to use to check synchronization word
        	HFRadioPacketLengthModes_t     HeaderType;        //!< If the header is explicit, it will be transmitted in the FLRC packet. If the header is implicit, it will not be transmitted.
        	uint8_t                      PayloadLength;     //!< Size of the payload in the FLRC packet
        	HFRadioCrcTypes_t              CrcLength;         //!< Size of the CRC block in the FLRC packet
        	HFRadioWhiteningModes_t        Whitening;         //!< Usage of whitening on payload and CRC blocks plus header block if header type is variable
        }Flrc;
        /*!
        ...
        struct
        {
        	HFRadioBleConnectionStates_t    ConnectionState;   //!< The BLE state
        	HFRadioBleCrcTypes_t            CrcLength;         //!< Size of the CRC block in the BLE packet
        	HFRadioBleTestPayloads_t        BleTestPayload;    //!< Special BLE payload for test purpose
        	HFRadioWhiteningModes_t         Whitening;         //!< Usage of whitening on PDU and CRC blocks of BLE packet
        }Ble;
...
typedef struct
{
	HFRadioPacketTypes_t                    packetType;        //!< Packet to which the packet status are referring to.
    union
    {
        struct
        {
...
public:
     ...
     */
    SX1280( HFRadioCallbacks_t *callbacks ):
...
private:
     ...
     */
    HFRadioPacketTypes_t PacketType;

    /*!
     * \brief Stores the current LORA bandwidth set in the radio
     */
    HFRadioLoRaBandwidths_t LoRaBandwidth;
...
protected:
     ...
     */
    void SetRangingRole( HFRadioRangingRoles_t role );
...
public:
     ...
     */
    virtual void WriteCommand( HFRadioCommands_t opcode, uint8_t *buffer, uint16_t size ) = 0;
     ...
     * \param [in]  size          Command parameters byte array size
     */
    virtual void ReadCommand( HFRadioCommands_t opcode, uint8_t *buffer, uint16_t size ) = 0;
     ...
     */
    virtual HFRadioStatus_t GetStatus( void );
     ...
     */
    void SetStandby( HFRadioStandbyModes_t mode );
     ...
     */
    void SetTx( HFTickTime_t timeout );
     ...
     */
    void SetRx( HFTickTime_t timeout );
     ...
     */
    void SetRxDutyCycle( HFRadioTickSizes_t periodBase, uint16_t periodBaseCountRx, uint16_t periodBaseCountSleep );
     ...
     */
    void SetPacketType( HFRadioPacketTypes_t packetType );
     ...
     */
    HFRadioPacketTypes_t GetPacketType( bool returnLocalCopy = false );
     ...
     */
    void SetTxParams( int8_t power, HFRadioRampTimes_t rampTime );
     ...
     */
    void SetCadParams( HFRadioLoRaCadSymbols_t cadSymbolNum );
     ...
     */
    void SetModulationParams( HFModulationParams_t *modParams );
     ...
     */
    void SetPacketParams( HFPacketParams_t *packetParams );
     ...
     */
    void GetPacketStatus( HFPacketStatus_t *packetStatus );
     ...
     */
    void Calibrate( HFCalibrationParams_t calibParam );
     ...
     */
    void SetRegulatorMode( HFRadioRegulatorModes_t mode );
     ...
     * \param [in]  offset        The address in FIFO where writting first byte (default = 0x00)
     */
    void SendPayload( uint8_t *payload, uint8_t size, HFTickTime_t timeout, uint8_t offset = 0x00 );
     ...
     */
    void SetLNAGainSetting( const HFRadioLnaSettings_t lnaSetting );
     ...
     *                             2: 24 bits, 3: 32 bits]
     */
    void SetRangingIdLength( HFRadioRangingIdCheckLengths_t length );
     ...
     */
    double GetRangingResult( HFRadioRangingResultTypes_t resultType );
     ...
     */
    void ForcePreambleLength( HFRadioPreambleLengths_t preambleLength );
};
