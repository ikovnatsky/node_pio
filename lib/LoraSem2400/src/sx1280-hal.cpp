...
void SX1280Hal::SetRadioCS(uint8_t val)
{
	if (val)
		 {
		digitalWrite(gRadioCS,HIGH);
		delay(1);
		 }
	else
		 digitalWrite(gRadioCS,LOW);
...
void SX1280Hal::WriteCommand( HFRadioCommands_t command, uint8_t *buffer, uint16_t size )
{
    WaitOnBusy( );
...
void SX1280Hal::ReadCommand( HFRadioCommands_t command, uint8_t *buffer, uint16_t size )
{
    WaitOnBusy( );
    ...
    if( RadioSpi != NULL )
    {
        ...
        {
            buffer[0] = RadioSpi->transfer( ( uint8_t )command );
            RadioSpi->transfer( 0 );
            RadioSpi->transfer( 0 );
        }
        else
        {
            RadioSpi->transfer( ( uint8_t )command );
            RadioSpi->transfer( 0 );
            for( uint16_t i = 0; i < size; i++ )
            {
                 buffer[i] = RadioSpi->transfer(0 ); // ik changed write to read
            }
        }
        SetRadioCS(1);
    }
