...
HFRadioStatus_t SX1280::GetStatus( void )
{
    uint8_t stat = 0;
    HFRadioStatus_t status;
...
void SX1280::SetStandby( HFRadioStandbyModes_t standbyConfig )
{
...
void SX1280::SetTx( HFTickTime_t timeout )
{
    uint8_t buf[3];
...
void SX1280::SetRx( HFTickTime_t timeout )
{
    uint8_t buf[3];
    buf[0] = timeout.PeriodBase;
    buf[1] = ( uint8_t )( ( timeout.PeriodBaseCount >> 8 ) & 0x00FF );
	...
	//printf("Setting timeout %x\n",timeout.PeriodBaseCount);

    // If the radio is doing ranging operations, then apply the specific calls
    // prior to SetRx
    if( GetPacketType( true ) == PACKET_TYPE_RANGING )
    {
    	//printf("Setting ranging\n");
...
void SX1280::SetRxDutyCycle( HFRadioTickSizes_t periodBase, uint16_t periodBaseCountRx, uint16_t periodBaseCountSleep )
{
    uint8_t buf[5];
...
void SX1280::SetPacketType( HFRadioPacketTypes_t packetType )
{
...
HFRadioPacketTypes_t SX1280::GetPacketType( bool returnLocalCopy )
{
    HFRadioPacketTypes_t packetType = PACKET_TYPE_NONE;
...
void SX1280::SetTxParams( int8_t power, HFRadioRampTimes_t rampTime )
{
    uint8_t buf[2];
...
void SX1280::SetCadParams( HFRadioLoRaCadSymbols_t cadSymbolNum )
{
...
void SX1280::SetModulationParams( HFModulationParams_t *modParams )
{
    uint8_t buf[3];
...
void SX1280::SetPacketParams( HFPacketParams_t *packetParams )
{
    uint8_t buf[7];
    // Check if required configuration corresponds to the stored packet type
    // If not, silently update radio packet type
...
void SX1280::ForcePreambleLength( HFRadioPreambleLengths_t preambleLength )
{
...
void SX1280::GetPacketStatus( HFPacketStatus_t *packetStatus )
{
    uint8_t status[5];
    ...
    switch( packetStatus->packetType )
    {
        ...
        case PACKET_TYPE_NONE:
            ...
            memset( packetStatus, 0, sizeof( HFPacketStatus_t ) );
...
void SX1280::Calibrate( HFCalibrationParams_t calibParam )
{
...
void SX1280::SetRegulatorMode( HFRadioRegulatorModes_t mode )
{
...
void SX1280::SendPayload( uint8_t *payload, uint8_t size, HFTickTime_t timeout, uint8_t offset )
{
...
void SX1280::SetLNAGainSetting( const HFRadioLnaSettings_t lnaSetting )
{
    /*
    switch(lnaSetting)
    {
...
void SX1280::SetRangingIdLength( HFRadioRangingIdCheckLengths_t length )
{
    switch( GetPacketType( true ) )
    {
        case PACKET_TYPE_RANGING:
...
double SX1280::GetRangingResult( HFRadioRangingResultTypes_t resultType )
{
    uint32_t valLsb = 0;
    double val = 0.0;
...
void SX1280::SetRangingRole( HFRadioRangingRoles_t role )
{
    uint8_t buf[1];
